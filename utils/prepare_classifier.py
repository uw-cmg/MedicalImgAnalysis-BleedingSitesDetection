#!/usr/bin/env python
"""
The scripts help prepare the training data for the classification of bleeding and
non bleeding patients.

Algorithm :
(1) Loops all Labellin result folders
(2) For each TIFF images
	(2.1) Converts to JPG
	(2.2) If find the same name CSV, label this image 1 which means bleeding
	(2.3) if not found, label this image 0 which means non bleeding
	(2.4) All label results written into lable.txt with the following format:
			JPG_Image_Name Label
"""
...
def generateTXT ( csvFile , txtFile ) :
	...
	with open ( txtFile , 'w' ) as txtfile :
		with open ( csvFile ) as csvfile :
			...
			for row in reader :
				bbox = calculateBoundingBoxes ( None , float ( row [ 'X' ] ) , float ( row [ 'Y' ] ) ,
...
def calculateBoundingBoxes ( label , x , y , w , h ) :
	...
	if label == None:
		return [ round ( Y1 , 2 ) , round ( X1 , 2 ) , round ( Y2 , 2 ) , round ( X2 , 2 ) ]
	else:
