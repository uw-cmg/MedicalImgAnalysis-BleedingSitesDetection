...
def get_dataset(dataset, args):
    ...
    val_metric = VOCMApMetric(iou_thresh=0.01, class_names=['bleeding_site'])
...
if __name__ == '__main__':
    ...
    # args.save_prefix = current_time + "lr:" + str(args.lr) + ",epoch:" + str(args.epochs)
    ...
    if args.syncbn and len(ctx) > 1:
        ...
        #net = get_model(net_name, pretrained_base=True, pretrained=True, classes = np.array(['bleeding_site']))
        net = get_model('yolo3_darknet53_coco', pretrained=True)
        net.reset_class(np.array(['bleeding_site']))
        print('intialize weights from a COCO pretrained model')
        async_net = net
