...
parser.add_argument("--weights_path", type=str, default='checkpoints/13_Jan_03_59/350_weights.pt', help="path to weights file")
...
for label in range(num_classes):
    ...
    # compute rcll and prcn
    rcll = true_positives / num_annotations
    prcn = true_positives / np.maximum(true_positives + false_positives, np.finfo(np.float64).eps)

    # compute average prcn
    average_prcn = compute_ap(rcll, prcn)
    average_prcns[label] = average_prcn

print("Average Precisions:")
for c, ap in average_prcns.items():
    print(f"+ Class '{c}' - AP: {ap}")

mAP = np.mean(list(average_prcns.values()))
print(f"mAP: {mAP}")
