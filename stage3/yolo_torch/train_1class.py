...
parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/"+experiment_ts, help="directory where model checkpoints are saved")
...
for epoch in range(opt.epochs):
    for batch_i, (_, imgs, targets) in enumerate(dataloader):
        ...
        if batch_i % 50==0:
            ...
            # print # replace logging.info if you want to print to the console
            logging.info(
                "[Epoch %d/%d, Batch %d/%d] [Losses: x %.5f, y %.5f, w %.5f, h %.5f, conf %.5f, cls %.5f, total %.5f, recall: %.5f, precision: %.5f]"
                % ( epoch, opt.epochs, batch_i, len(dataloader), model.losses["x"], model.losses["y"], model.losses["w"], model.losses["h"], model.losses["conf"], model.losses["cls"], loss.item(), model.losses["recall"]
    ...
    if epoch % opt.checkpoint_interval == 0:
        torch.save(model.state_dict(), "%s/%d_weights.pt" % (opt.checkpoint_dir, epoch))
